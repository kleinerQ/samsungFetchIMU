...
bool create_bluetooth_le_advertiser()
{
	...
	if(!set_bluetooth_le_advertising_gatt_sensorService_uuid(bluetooth_le_advertiser_handle))
	{
		dlog_print(DLOG_ERROR, LOG_TAG, "%s/%s/%d: Failed to add a GATT sensorService UUID to advertise data.", __FILE__, __func__, __LINE__);
		return false;
	}
	else
		dlog_print(DLOG_INFO, LOG_TAG, "%s/%s/%d: Succeeded in adding a GATT sensorService UUID to advertise data.", __FILE__, __func__, __LINE__);




	if(!set_bluetooth_le_advertising_manufacturer_data())
	{
		dlog_print(DLOG_ERROR, LOG_TAG, "%s/%s/%d: Failed to set_bluetooth_le_advertising_manufacturer_data.", __FILE__, __func__, __LINE__);
		return false;
	}
	else
		dlog_print(DLOG_INFO, LOG_TAG, "%s/%s/%d: Succeeded in sset_bluetooth_le_advertising_manufacturer_data.", __FILE__, __func__, __LINE__);
...
union SamsungWatchToPhoneAdvertisingData
{
    uint8_t packed[2];
#pragma pack(push,1) // prevent padding
    struct L2CapDataRssiModel
    {
        uint16_t unitID;
    } dataSplit;
#pragma pack(pop)
...
//	timeinfo = localtime ( &rawtime );
	...
	union SamsungWatchToPhoneAdvertisingData manufacturer_data;
	manufacturer_data.dataSplit.unitID = RandomManufactureData;
	int ret = bt_adapter_le_add_advertising_manufacturer_data(bluetooth_le_advertiser_handle,
				                                                  BT_ADAPTER_LE_PACKET_ADVERTISING,
																  0,
																  (const char *) manufacturer_data.packed,
																  sizeof(union SamsungWatchToPhoneAdvertisingData));
